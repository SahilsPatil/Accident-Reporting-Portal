...
export default function UserManagament() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch(url.URL + '/api/users/all');
                if (!response.ok) {
                            ...
                            <div className="dashboard_users_users">
                                ...
                                <div className="dashboard_users_users_users">
                                    <UserDataTitles />
                                    {
                                        userData.map( (e) => {
                                            if (e.role == 'admin') {
                                                return (<UserData id={e._id} name={e.firstName} email={e.email} location={e.Location} role={e.role} />)
                                            }
                                        })
                                    }
                                    {components}
                                    ...
                                    <UserDataTitles />
                                    {
                                        userData.map((e) => {
                                            if (e.role == 'police' || e.role == 'ambulance' || e.role == 'fire') {
                                                return (<UserData id={e._id} name={e.firstName} email={e.email} location={e.location.lat+" , "+e.location.lng} role={e.role} />)
